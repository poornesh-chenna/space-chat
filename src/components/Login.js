...
function Login(props) {
  ...
  const log = async (event) => {
    ...
    await axios
      .post('http://localhost:5000/login', user, headers)
      .then((res) => {
        console.log('Success')
        console.log(res)
      ...
      .catch((error) => {
        ...
        if (error.response.data === 'Please register') setisRegistered(false)
      })
  }
  ...
  return (
    <div className="forms d-flex flex-row min-vh-100 justify-content-center align-items-center">
      <Form className="m-5">
        ...
        <Link className="mt-2 d-block text-light" to="/Register" role="button">
          Not yet Registered?
        </Link>
      </Form>
